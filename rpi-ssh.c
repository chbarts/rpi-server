/* Code that automatically opens an SSH session to the Raspberry Pi running the server software. */
...
int main(void)
{
    ...
    char message[sizeof(data) + sizeof(crc)], addrstr[INET6_ADDRSTRLEN],
        cmd[sizeof(COMMAND) + INET6_ADDRSTRLEN + 1];
    int sock, ret;
    ...
    snprintf(cmd, sizeof(cmd), COMMAND, addrstr);
    ret = system(cmd);
